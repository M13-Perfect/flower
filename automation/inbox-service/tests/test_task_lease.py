...
def test_order_in_scope_time_window():
    ...
    assert order_in_scope(c, paid_at=datetime(2026, 6, 20), now=NOW) is True
    assert order_in_scope(c, paid_at=datetime(2026, 6, 18), now=NOW) is False  # 早于下界（历史）
    assert order_in_scope(c, paid_at=datetime(2026, 6, 22), now=NOW) is False  # 晚于上界
...
def test_order_in_scope_shop_filter():
    ...
    assert order_in_scope(c, paid_at=datetime(2026, 6, 20), shop="shopA", now=NOW) is True
    assert order_in_scope(c, paid_at=datetime(2026, 6, 20), shop="shopC", now=NOW) is False

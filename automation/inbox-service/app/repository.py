...
def bind_order_template(
    ...
    *,
    ...
    template_sha256: str,
    status: str = TEMPLATE_BINDING_BOUND,
) -> Order | None:
    """给已存在的订单绑定 GIMP 模板：写四个 template_* 字段并返回该单。订单不存在→None（路由层 404）。

    绑定走这条唯一路径（入库 upsert 不碰 template_*，重导入天然保留绑定）。flower 桌面端随后读
    Order.to_dict() 拿 template_id/version/sha256 交给 GIMP 编辑器。status 缺省 bound（已绑定有效）。
    """
    order = session.get(Order, order_id)
    if order is None:
        return None
    order.template_id = template_id
    order.template_version = template_version
    order.template_sha256 = template_sha256
    order.template_binding_status = status
    session.flush()
    return order
...
def upsert_order(session: Session, payload: OrderPayload, raw_json: str) -> tuple[Order, bool, bool, bool]:
    ...
    # GIMP 模板绑定**不经此路径**：入库报文（OrderPayload）不带 template_* 字段，故重导入（扩展重抓）
    # 天然保留库里已有的绑定——绝不在这里把 template_id/version/sha256 抹成 None。绑定只走 bind_order_template。

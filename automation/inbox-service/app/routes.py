...
@router.post("/inbox/orders/{order_id}/template-binding")
def bind_template_route(order_id: str, body: TemplateBindingBody, request: Request) -> dict:
    """给订单绑定 GIMP 模板，返回更新后的订单（含 template_* 字段）。订单不存在→404。

    flower 桌面端确定订单用哪套 GIMP 模板后调它持久化绑定；随后任何端读 Order.to_dict() 都带上
    template_id/version/sha256，把订单交给对应 GIMP 模板编辑器。绑定不经入库 upsert（重导入天然保留）。
    """
    factory = request.app.state.session_factory
    with session_scope(factory) as session:

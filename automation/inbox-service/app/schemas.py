...
class TemplateBindingBody(BaseModel):
    """flower 桌面端给订单绑定 GIMP 模板（POST /inbox/orders/{order_id}/template-binding）。

    sha256 是 64 位十六进制（GIMP 模板文件指纹）；status 缺省 bound。绑定后桌面端读
    Order.to_dict() 拿 template_id/version/sha256 交给 GIMP 编辑器。
    """
    ...
    template_sha256: str = Field(min_length=1, max_length=64)
    status: Literal["bound", "invalid", "unbound"] = "bound"

...
> **2026-06-23 · 引用字段 UI 修复 ✅逻辑+自动化测试 / ⬜真机 GUI 回归**  
> **背景**：当前生产链路仍是 Tkinter/CustomTkinter 桌面端 `ui_app.py`；可引用字段后端 `prompt_references.py` 已用稳定 `{{field:<uuid>}}` / `{{source:order_information}}` token 存储和解析，本轮只修 UI 集成层显示/状态同步。  
> **本次改动**：`prompt_references.py` 新增 `iter_template_segments()`，`render_template_view()` 复用同一段映射，未知字段可见显示为 `/无效字段`，不再暴露 UUID。`ui_app.py` 的背景提示词编辑器改为加载时把 token 渲染成可见 `/字段名`/`/订单信息`，用 Tk Text tag 保存 `ref::<uuid>` / `src::<key>`，回读 `_current_prompt_template_text()` 时再恢复 canonical token；斜杠菜单数据拆成 `label`（可含 `#序号`）、`display_name`（插入可见文本）、`insert`（内部 token）、`ref_kind/ref_id`。字段名输入新增 `<FocusOut>` 保存，未变化不写盘，失焦重名/空名只回状态栏不弹窗；`_reference_fields_from_field_defs()` 补读 `name_var`，避免未按 Enter 的改名在增删/失焦持久化时丢失。  
> **测试/验证**：新增 `tests/test_template_segments.py` 和 `tests/test_reference_field_ui_mapping.py` 覆盖分段映射、编辑器可见名不显 token、tag 回读 token、斜杠菜单 label/display/token 分离、未提交 `name_var` 持久化。验证：`pytest tests/test_template_segments.py tests/test_reference_field_ui_mapping.py tests/test_reference_field_system.py tests/test_config_store.py -q` = 45 passed；`pytest tests/test_orders_multi.py tests/test_inbox_poller.py -q` = 26 passed；`pytest tests/test_ui_app.py -k "field_defs_persist_and_reload or field_instructions_drive_ai_system_prompt or add_field_sequence_stays_unique_after_delete" -q` = 3 skipped（当前环境无 Tk display）；`py_compile` 和 `ruff check prompt_references.py ui_app.py tests/test_template_segments.py tests/test_reference_field_ui_mapping.py` 通过。pytest 退出时仍有 Windows 临时目录 `pytest-current` PermissionError atexit 清理提示，测试退出码为 0。  
> **真机待验**：完全关 App 重开，在管理员端把 `#3 info3` 改名为「生日月份」不按 Enter，直接点背景提示词输入 `/`，菜单应立即显示新名；选中后编辑器可见 `/生日月份`（无 `#3`、无 UUID），保存/重启后仍显示友好名且预览 `[template]` 不含 token；解析时最终提示词展开为字段 prompt；删除 `#2` 后 `#3` 不重排，新建继续 `#4`。  
> **已知/取舍**：本轮保留 `product.prompt_template` 内部 token 格式，不把稳定 ID 换成字段名；MVP 未做 chip 原子编辑护栏，若用户在已插入的 `/字段名` tag 内局部编辑，回读仍按该 tag 的原引用处理；历史 `info3` 不自动迁移，靠用户手动改名；`background_prompt` 兼容字段仍可能存 token 字符串，当前真实入口以 `prompt_template` 为准。

> **2026-06-23 · 追加修复：斜杠菜单「引用后再接引用」不触发 / `//` 双斜杠 ✅逻辑+单测 / ⬜真机**
> **症状**：在一个 `/字段名` chip 后紧接着打 `/` 想再引用，弹窗不出现（用户感觉「检测没刷新」）；遗留游离 `/` 紧贴 chip，回读成 `…/{{field:..}}…`，重渲染拆成 文本`/`+chip`/名称` = `//名称`，存盘后每次重渲都复现。
> **根因**：`_slash_query_before_cursor`（`ui_app.py`）的闸 `if slash_at>0 and not line[slash_at-1].isspace(): return None`——chip 可见文本末字符（如「体」）非空白，紧跟其后的 `/` 被静默吞掉。检测每次 KeyRelease 都跑，只是被这道闸拦了。
> **改动**：闸加第三放行条件「前一字符落在 chip tag 内」：`... and not self._char_in_chip(slash_at-1)`；新增 `_char_in_chip(col)` 查底层 tk.Text 该列是否带 `chip` tag。弹窗正常触发后，选词时 `_insert_slash_candidate` 的 `box.delete(slash_start,"insert")` 顺手吃掉那个 `/` → 不再产生游离斜杠 → `//` 从源头消失；引用后引用变成真 chip → 预览正常展开（之前因没变 chip 而原样 `/名称` 贴在一起，看着像「换行没生效」）。
> **决策（用户拍板「不要」）**：`内容/字体` 这种跟在普通非空白文字后的 `/` **不**自动触发（与 `2cm/3cm`、`and/or`、日期 `6/23` 无法区分），需手动加空格或另起行——故意保留。已存盘的旧 `//` 字面不自动清，手删一次即可。
> **测试**：`tests/test_reference_field_ui_mapping.py` 加 `test_slash_popup_triggers_after_chip_but_not_after_plain_text`；并给该文件 `_RawText` fake 补 `edit_reset` no-op（修一个与本次无关的既有红：`_render_template_into_editor` 早先加的 `edit_reset()` fake 没跟上）。`py_compile ui_app.py` 通过；`pytest tests/test_reference_field_ui_mapping.py -q` = 6 passed。
> **⬜真机待验**：关 App 重开，背景框插入一个 `/字段名` 后紧接着打 `/` → 弹窗应立即出现、选词后是两个相邻 chip 无 `//`；普通文字后打 `/` 仍不弹。

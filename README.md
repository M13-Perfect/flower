...
- `文件 -> 导入 -> 导入素材...` 会按选中文件后缀识别导入类型：TTF/OTF 导入为当前字体文本，SVG/PNG/JPG/JPEG/WEBP/BMP 导入为当前花朵素材。
...
- 输出支持勾选 `PNG`、`SVG`、`DXF`，默认建议使用 `SVG` + `DXF`；位图素材导出 SVG 时会以图片嵌入并标注不是纯矢量，DXF 仅支持纯矢量 SVG 素材。

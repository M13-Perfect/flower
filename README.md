...
- 功能区保留高频动作：订单备注、导入、解析、素材/字体选择、布局参数、人工确认生成。
- `文件 -> 导入 -> 导入素材...` 会按选中文件后缀识别导入类型：TTF/OTF 导入为当前字体，SVG/PNG/JPG/JPEG/WEBP/BMP 只会选为待添加素材；只有点击“添加素材为新图层”才会创建新的 ImageLayer。
- 素材扫描会生成通用 `asset_key` 和展示名，可按素材文件名匹配，不再只依赖固定月份与 `flower 1-2`。
- 实时画板使用多图层 Document 作为唯一画布数据源，并使用 SVG 预览缓存；素材文件和布局未变化时不会重复解析 SVG。
- `文件 -> 设置... -> 输出设置` 可配置输出格式、输出路径和输出分辨率（画布宽/画布高）。

...
- **⚠️ 发现一个与爱心无关的既有 BUG（未修）**：`ui_app._load_field_defs_into_self`（ui_app.py:4226–4282）仍读 `product.reference_fields`，但 `ProductConfig`（config_store.py:111）早已把提示词搬进 `prompts.db`、只留 `prompt_set_id`，**没有 `reference_fields` 字段** → `BirthFlowerApp.__init__` 在「Tk display 可用」时必崩 `AttributeError`（headless 时 test_ui_app 整体 skip 故被掩盖，这也是 CURRENT_TASKS「622/0」基线只在无显示器下成立的原因）。本机带显示器跑全量会看到 ~53 个 test_ui_app 失败，**全是这一处 reference_fields**，与本次爱心/字形改动无关（未碰 config_store/ProductConfig/该方法）。**疑似也会影响真机 App 启动**——建议下个会话把 ui_app 那几处改读 `prompt_set.reference_fields`（参照 ui_app.py:3697/3967 已有的 `_active_prompt_set()` 用法）。

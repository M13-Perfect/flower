...
def with_product_status(
    config: AppConfig, status: str, *, product_id: str | None = None
) -> AppConfig:
    """设置产品状态（active|disabled）。停用=软删；启用=恢复。返回新配置（不可变）。"""
    if status not in ("active", "disabled"):
        raise ValueError(f"非法产品状态：{status!r}")
    target_id = product_id or config.active_product_id
    products = tuple(
        replace(product, status=status) if product.id == target_id else product
        for product in config.products
    )

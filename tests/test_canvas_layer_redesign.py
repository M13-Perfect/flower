...
def test_inline_text_edit_pushes_one_history_snapshot_and_cancel_drops_it():
    class FakeEditor:
    ...
    layer = add_text_layer(app.document, "A", width=20, height=10)
    app.inline_text_entry = FakeEditor("AB")
    app.inline_text_layer_id = layer.id
    app.inline_text_original_text = "A"
    app.inline_text_history_pushed = False
    app.inline_text_original_box = (layer.x, layer.y, layer.width, layer.height, layer.text_box_width, layer.text_box_height)
    app.inline_text_is_closing = False
    app.inline_text_render_after_id = None
    app.inline_text_window = None
    app.floating_text_editor = None
    app.preview_canvas = None
    app.history_manager = HistoryManager()
    app.layer_text_var = FakeVar()
    app.status_var = FakeVar()
    app._place_inline_text_editor = lambda: None
    app._schedule_canvas_render = lambda: None
    app._refresh_layers_panel = lambda: None
    app._redraw_preview = lambda: None

    assert app._on_inline_text_modified(SimpleNamespace()) == "break"
    app.inline_text_entry.value = "ABC"
    app.inline_text_entry.modified = True
    assert app._on_inline_text_modified(SimpleNamespace()) == "break"

    assert len(app.history_manager.undo_stack) == 1
    assert layer.original_text == "ABC"

    assert app._cancel_inline_text_edit() == "break"

    assert app.history_manager.undo_stack == []
    assert layer.original_text == "A"
...
def test_layer_rows_show_content_and_inline_edit_updates_text():
    config = AppConfig(products=(ProductConfig(id="p1", name="P1"),), active_product_id="p1")
    app = _fake_app(config)
    app.layer_text_var = FakeVar()
    tree = FakeTree()
    document = Document()
    text = add_text_layer(document, "致我最爱的宝贝")
    text.glyph_overrides = {0: {"variant": "x"}}
    image = add_image_layer(document, Path("rose.svg"), name="文件名", material_name="玫瑰花束-红")
    app.document = document
    app.layers_tree = tree

    # 文字层显示文本内容、图片层显示引用的素材名（而非图层名/文件名）。
    assert app._layer_main_text(text) == "致我最爱的宝贝"
    assert app._layer_main_text(image) == "玫瑰花束-红"

    app._render_layers_tree()
    # #0 列文本含真实内容；行值 4 列(资源/👁/🔒/🗑)，资源在最前、🗑 永远在最后一列。
    assert "致我最爱的宝贝" in tree.item(text.id, "text")
    assert "玫瑰花束-红" in tree.item(image.id, "text")
    assert len(tree.item(text.id, "values")) == 4
    assert tree.item(text.id, "values")[3] == "🗑"
    # 资源格(第 1 列)：图片层显当前素材名，可点开选库/素材。
    assert tree.item(image.id, "values")[0] == "玫瑰花束-红"

    # 行内编辑写回：内容更新、字形绑定清空、留下撤销点。
    app._set_text_layer_content(text, "生日快乐")
    assert (text.original_text, text.text, text.render_text) == ("生日快乐", "生日快乐", "生日快乐")
    assert text.glyph_overrides == {}
    assert app.history_manager.undo_stack

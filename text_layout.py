...
from heart_symbol import HEART_ASPECT, HEART_VIEW_H
...
# Font 4 等“独立爱心”字体：末尾爱心高度占字号比例 + 与末字水平间隙占字号比例（可调，真机锁定）。
ENDING_HEART_SIZE_RATIO = 0.62
ENDING_HEART_GAP_RATIO = 0.12
# 爱心挂在末行右侧占的“额外推进量”占字号比例：参与名字适配，保证名字+爱心一起装进框、右侧不裁切。
ENDING_HEART_ADVANCE_RATIO = ENDING_HEART_GAP_RATIO + ENDING_HEART_SIZE_RATIO * HEART_ASPECT
...
def fit_text_box(
    ...
    *,
    ...
    ending_advance_ratio: float = 0.0,
) -> BoxTextFit:
    ...
    ending_advance_ratio>0 时（Font 4 末尾独立爱心）：在末行右侧预留爱心推进量（占字号比例），
    让名字+爱心一起 contain-fit、末行按整体居中、不裁切。默认 0.0 → 与原逻辑字节一致。
    """
    ...
    if ptype == "message":
        ...
        ending_advance_ratio = 0.0  # 祝福语不挂末尾爱心
    else:
        font_size, lines = _fit_name_layout(raw, box_w, box_h, font_path, font_size_cap, ending_advance_ratio)
    # 末尾爱心占位（像素）：参与末行居中与超框判定；ratio=0 时下游与原逻辑字节一致。
    ending_advance_px = max(0.0, float(ending_advance_ratio)) * font_size
    origins, ink_union = _placement_origins_ls(
        ...
        letter_spacing=letter_spacing, ending_advance=ending_advance_px,
    )
...
# 字号驱动文本框：框略放大一点，保证 fit_text_box 的自适应字号 ≥ 目标字号，
# 再由 font_size_cap 精确锁定到目标字号（cap 封顶），从而渲染真实字号==目标字号。
_BOX_FONT_SLACK = 1.04


def text_box_size_for_font(
    text: str,
    font_size: int,
    font_path: Path | str | None = None,
    *,
    ...
    """按**目标字号反推文本框 (width, height, clamped)**——与 fit_text_box 的自适应比例互逆。

    用法：把返回的 (w, h) 写回图层 text_box_width/height，再正常走
    ``fit_text_box(text, w, h, font_path, font_size_cap=font_size)``，渲染出的真实字号即等于 font_size
    （框按比例放大 _BOX_FONT_SLACK，使自适应≥目标、由 cap 精确封顶）。预览与导出共读同一框，所见即所得。

    名字：单行墨迹太宽（净宽超 max_width）时自动均衡断到 ≤NAME_MAX_LINES 行；框宽高都随字号线性长大。
    任一维超过 max_width/max_height 时**封顶到上限**并返回 clamped=True——此时按该框 fit 会把字号缩到装下，
    调用方应据 clamped 给「已封顶到画布安全区」提示。
    """
    font_size = max(1, int(font_size))
    max_w = max(1.0, float(max_width))
    max_h = max(1.0, float(max_height))
    raw = "" if text is None else str(text)
    clean = raw.strip()
    if not clean:
        w = min(max_w, math.ceil(font_size * _BOX_FONT_SLACK))
        h = min(max_h, math.ceil(font_size / NAME_HEIGHT_RATIO * _BOX_FONT_SLACK))
        return int(w), int(h), bool(w >= max_w or h >= max_h)

    ptype = _resolved_personalization_type(clean, personalization_type)
    if ptype == "message":
        # 祝福语字号本就硬封顶 160（见 _fit_message_box）：用满宽 + 按行高堆高，超限封顶。
        eff = min(font_size, 160)
        usable_w = max(1.0, max_w * (1.0 - 2 * NAME_SIDE_PAD_RATIO))
        lines = _wrap_text(clean, eff, usable_w, font_path)
        box_h = math.ceil(max(1, len(lines)) * eff * LINE_HEIGHT_RATIO * _BOX_FONT_SLACK)
        clamped = bool(font_size > 160 or box_h > max_h)
        return int(min(math.ceil(max_w), max_w)), int(min(box_h, max_h)), clamped

    adv_ref = max(0.0, float(ending_advance_ratio)) * _REF_FONT_SIZE

    def _box_for_lines(lines: list[str]) -> tuple[float, float]:
        measured = [measure_text_ink_bbox(line, _REF_FONT_SIZE, font_path) for line in lines]
        last = len(measured) - 1
        widths = [m.width + (adv_ref if i == last else 0.0) for i, m in enumerate(measured)]
        max_ref_w = max([w for w in widths if w > 0] or [1.0])
        # 净宽留 NAME_SIDE_PAD_RATIO 边距 → 反推框宽；墨迹高/块高占框高 NAME_HEIGHT/BLOCK_RATIO → 反推框高。
        box_w = (font_size * max_ref_w / _REF_FONT_SIZE) / (1.0 - 2 * NAME_SIDE_PAD_RATIO)
        if len(lines) == 1:
            ink_h_ref = max([m.height for m in measured] or [1.0])
            box_h = font_size * ink_h_ref / (_REF_FONT_SIZE * NAME_HEIGHT_RATIO)
        else:
            heights = [m.height for m in measured if m.height > 0] or [1.0]
            block_h_ref = (len(lines) - 1) * _REF_FONT_SIZE * LINE_HEIGHT_RATIO + max(heights)
            box_h = font_size * block_h_ref / (_REF_FONT_SIZE * NAME_BLOCK_HEIGHT_RATIO)
        return box_w, box_h

    words = clean.split()
    box_w, box_h = _box_for_lines([clean])  # 先按单行
    if box_w * _BOX_FONT_SLACK > max_w and len(words) > 1:
        # 单行太宽 → 均衡断行（最多 NAME_MAX_LINES），框转为更窄更高。
        wrapped = _balanced_wrap(words, min(NAME_MAX_LINES, len(words)), font_path)
        box_w, box_h = _box_for_lines(wrapped)

    box_w *= _BOX_FONT_SLACK
    box_h *= _BOX_FONT_SLACK
    clamped = bool(box_w > max_w or box_h > max_h)
    return int(min(math.ceil(box_w), max_w)), int(min(math.ceil(box_h), max_h)), clamped


def place_ending_heart(
    fit: BoxTextFit,
    font_path: Path | str | None,
    *,
    size_ratio: float = ENDING_HEART_SIZE_RATIO,
    gap_ratio: float = ENDING_HEART_GAP_RATIO,
) -> tuple[float, float, float] | None:
    """末尾独立爱心在 box 本地坐标的 (x, y, scale)：紧贴末行末字右侧、竖直居中于末行墨迹。

    预览(text_renderer)与导出(desktop_export→svg/dxf)共用本函数，三端几何一致。
    爱心高 = size_ratio*font_size，scale = 高/HEART_VIEW_H；x = 末行墨迹右缘 + gap。
    末行无可见墨迹（空行）返回 None（无处可缀）。
    """
    if not fit.lines or not fit.origins:
        return None
    last_line = fit.lines[-1]
    pen_x, baseline_y = fit.origins[-1]
    ink = measure_text_ink_bbox(last_line, fit.font_size, font_path)
    if ink.width <= 0 or ink.height <= 0:
        return None
    ascent, _descent = _font_ascent_descent(font_path, fit.font_size)
    heart_h = size_ratio * fit.font_size
    scale = heart_h / HEART_VIEW_H
    gap = gap_ratio * fit.font_size
    x = pen_x + ink.right + gap
    # 末行墨迹在 box 竖直区间：ink.* 在 la 空间，基线在 la 的 ascent 处，故减 ascent 平移到 box。
    ink_top_box = baseline_y + (ink.top - ascent)
    ink_bottom_box = baseline_y + (ink.bottom - ascent)
    y = (ink_top_box + ink_bottom_box) / 2 - heart_h / 2
    return (x, y, scale)
...
def _fit_name_font_size(
    ...
    ending_advance_ratio>0 时把末尾爱心推进量并进宽度，确保名字+爱心一起装下；0 时无影响。
    """
    box_w = max(1.0, float(box_w))
    box_h = max(1.0, float(box_h))
    adv_ratio = max(0.0, float(ending_advance_ratio))
    ...
    # 末尾爱心推进量折算到参考字号、并进宽度，让名字+爱心一起 fit。
    fit_by_width = _REF_FONT_SIZE * box_w / (ink.width + adv_ratio * _REF_FONT_SIZE)
    ...
    for _ in range(8):
        ...
        # 末尾爱心也算进有效宽度（随字号线性），防止名字单独装下、加爱心却溢出。
        eff_width = ink_now.width + adv_ratio * size
        if eff_width <= box_w and ink_now.height <= box_h:
            break
        shrink = min(box_w / max(1.0, eff_width), box_h / max(1.0, ink_now.height))
...
def _fit_lines_name_size(
    ...
    measured = [measure_text_ink_bbox(line, _REF_FONT_SIZE, font_path) for line in lines]
    last = len(measured) - 1
    line_widths = [m.width + (adv_ref if i == last else 0.0) for i, m in enumerate(measured)]
    widths = [w for w in line_widths if w > 0]
    heights = [m.height for m in measured if m.height > 0]
    if not widths or not heights:
        return _fit_name_font_size(" ".join(lines), box_w, box_h, font_path, cap, ending_advance_ratio)
...
def _fit_name_layout(
    ...
    ending_advance_ratio>0 时，末行宽度计入末尾爱心推进量（名字+爱心一起 fit）。
    """
    box_w = max(1.0, float(box_w))
    box_h = max(1.0, float(box_h))
    raw = str(text)
    ...
    if len(words) <= 1:
        return _fit_name_font_size(raw, usable_w, box_h, font_path, cap, ending_advance_ratio), [raw]
    ...
    for line_count in range(1, min(NAME_MAX_LINES, len(words)) + 1):
        if line_count == 1:
            lines = [raw]
            size = float(_fit_name_font_size(raw, usable_w, box_h, font_path, cap, ending_advance_ratio))
        else:
            lines = _balanced_wrap(words, line_count, font_path)
            size = float(_fit_lines_name_size(lines, usable_w, box_h, font_path, cap, ending_advance_ratio))
    ...
    while size_int > 1 and not _lines_fit(best_lines, size_int, slot, font_path, ending_advance_ratio):
        size_int -= 1
...
def _placement_origins_ls(
    ...
    *,
    ...
    ending_advance>0（末尾爱心）只加在**最后一行**：该行按“名字+爱心”整体居中，
    且 ink_union 计入爱心右边，使 did_fit 反映爱心是否也装下。默认 0 → 与原逻辑字节一致。
    """
    box_w = max(1.0, float(box_w))
    box_h = max(1.0, float(box_h))
    font_size = max(1, int(font_size))
    ...
    for index, line in enumerate(lines):
        ...
        line_advance = float(ending_advance) if index == last_index else 0.0  # 爱心只挂最后一行
        ...
        if align == "center":
            pen_x = (box_w - spread_width - line_advance) / 2 - ink.left
        elif align == "right":
            pen_x = box_w - ink.right - spacing_pad - line_advance
        else:
            pen_x = -ink.left
        per_line.append((pen_x, baseline_la))
        union_left = min(union_left, pen_x + ink.left)
        union_right = max(union_right, pen_x + ink.left + spread_width + line_advance)
...
def _lines_fit(lines: list[str], font_size: int, slot: Bounds, font_path: Path | str | None, ending_advance_ratio: float = 0.0) -> bool:
    if _line_height(font_size) * len(lines) > slot.height:
        return False
    adv = max(0.0, float(ending_advance_ratio)) * font_size  # 末尾爱心推进量，仅加在最后一行
    last = len(lines) - 1
    return all(
        measure_text_ink_bbox(line, font_size, font_path).width + (adv if i == last else 0.0) <= slot.width
